...
export default function ProfilePage() {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      setIsSearching(true);
      setShowSearchResults(true); // Show dropdown immediately when searching
  ...
  return (
    <div class="profile-container">
      ...
      <nav class="app-bar">
        ...
        <button class="nav-button" onClick={() => location.route("/search")}>
          Search
        </button>
        <button
          class="nav-button"
          ...
          {showSearchResults && (
            <div class="search-results-dropdown">
              {isSearching
                ...
                ? (
                  searchResults.map((result) => (
                    <div
                      ...
                      onClick={() => handleSelectSet(result.id)}
                    >
                      <div class="search-result-title">{result.title}</div>
                      <div class="search-result-owner">by {result.owner}</div>
                      {result.card && (
                        <div class="search-result-preview">
                          {result.card.front && (
                            <span>"{result.card.front}"</span>
                          )}
                        </div>
                      )}
                    </div>
                ...
                : (
                  <div class="search-result-item no-results">No sets found</div>
                )}
            </div>
          )}
        </div>
        <button
          ...
          onClick={() => alert("Create new set - Coming soon!")}
        >
          Create New Set
        </button>
        <button
      ...
      <div class="profile-content">
        ...
        <div class="profile-left">
          <div class="profile-avatar-large">
            <span class="avatar-letter-large">
              {userData.name.charAt(0).toUpperCase() || "P"}
            </span>
          </div>
          <button
            class="update-icon-link"
            onClick={() => alert("Update profile icon - Coming soon!")}
          >
            Update Profile Icon
          </button>
          ...
          <div class="profile-user-info">
            <h1>{userData.name}</h1>
            <span class="edit-icon" onClick={() => setIsEditing(!isEditing)}>
              ✏️
            </span>
          ...
          {isEditing
            ...
            : (
              <div class="bio-display">
                {userData.bio ||
                  "Write a little bit about yourself for others to see!"}
              </div>
            )}
